 ...
 * @returns {Error}
 */
const uvExceptionWithHostPort = hideStackFrames(function uvExceptionWithHostPort(
  err,
  syscall,
  address,
  port
...
const fatalExceptionStackEnhancers = {
  ...
  afterInspector(error) {
    ...
    const colors =
      useColors && (internalBinding('util').guessHandleType(2) === 'TTY' || defaultColors)
    try {
  ...
  (moduleURL, realIntegrities) => {
    ...
    if (realIntegrities.size) {
      const sri = ArrayPrototypeJoin(
        ArrayFrom(realIntegrities.entries(), ({ 0: alg, 1: dgs }) => `${alg}-${dgs}`),
        ' '
      )
...
E(
  ...
  (...args) => {
    ...
    args = ArrayPrototypeMap(args, (a) =>
      ArrayIsArray(a) ? ArrayPrototypeJoin(ArrayPrototypeMap(a, wrap), ' or ') : wrap(a)
    )
    switch (len) {
      case 1:
        msg += `${args[0]} argument`
...
E(
  ...
  (url, supported) => {
    let msg = `Only URLs with a scheme in: ${ArrayPrototypeJoin(
      supported,
      ', '
...
E(
  ...
  (filename) =>
    ...
    (StringPrototypeStartsWith(filename, 'file://') ? ' Wrap file:// URLs with `new URL`.' : '') +
