...
export default class GoTrueClient {
  ...
  async signInWithWechat(credentials: SignInWithWechatCredentials): Promise<AuthResponse> {
    try {
      ...
      if ('code' in credentials) {
        ...
        throw new AuthInvalidCredentialsError('code是必填项')
      }
    ...
    try {
      ...
      if ('code' in credentials) {
        ...
        const { data, error: userError } = await _request(
          this.fetch,
          'GET',
          `${this.url}/wechat_mini/bind_phone?code=${code}`,
          {
            headers: this.headers,
    ...
    try {
      ...
      if ('code' in credentials) {
        ...
        const { data, error: userError } = await _request(
          this.fetch,
          'GET',
          `${this.url}/wechat_mini/bind_account?code=${code}`,
          {
            headers: this.headers,
            jwt: session.access_token,
            xform: _userResponse,
  ...
  async getUnlimitedQRCode(credentials: unlimitedQRCodeCredentials) {
    try {
      let res
      const { page, scene, check_path, env_version, width, auto_color, line_color, is_hyaline } =
        credentials
