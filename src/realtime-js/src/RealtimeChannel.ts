...
export default class RealtimeChannel {
  ...
  subscribe(
    ...
    if (this.joinedOnce) {
      ...
      const config = {
        broadcast,
        presence,
        postgres_changes: this.bindings.postgres_changes?.map((r) => r.filter) ?? [],
      }
  ...
  async send(
    ...
    if (!this._canPush() && args.type === 'broadcast') {
      ...
      return new Promise((resolve) => {
        const push = this._pushEvent(args.type, args, opts.timeout || this.timeout)
  ...
  /** @internal */
  _pushEvent(event: string, payload: { [key: string]: any }, timeout = this.timeout) {
    if (!this.joinedOnce) {
