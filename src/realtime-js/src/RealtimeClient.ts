...
export default class RealtimeClient {
  ...
  constructor(endPoint: string, options?: RealtimeClientOptions) {
    ...
    this.decode = options?.decode ? options.decode : this.serializer.decode.bind(this.serializer)
  ...
  setAuth(token: string | null): void {
    ...
    this.channels.forEach((channel) => {
      ...
      if (channel.joinedOnce && channel._isJoined()) {
        channel._pushEvent(CHANNEL_EVENTS.access_token, { access_token: token })
      }
    })
  }
...
class WSWebSocketDummy {
  ...
  constructor(address: string, _protocols: undefined, options: { close: Function }) {
    this.url = address
